...
float BME280::readTemperature() {
    ...
    int32_t adc_T = ((uint32_t)read8(0xFA) << 12) |
                    ((uint32_t)read8(0xFB) << 4)  |
                    (read8(0xFC) >> 4);
...
float BME280::readPressure() {
    ...
    int32_t adc_P = ((uint32_t)read8(0xF7) << 12) |
                    ((uint32_t)read8(0xF8) << 4)  |
                    (read8(0xF9) >> 4);
    ...
    if (var1 == 0) return lastPressure; //--> avoid divide by zero!!!!
    ...
    float pressure = (float)p / 25600.0f;
...
float BME280::readHumidity() {
    ...
    int32_t adc_H = ((uint32_t)read8(0xFD) << 8) | read8(0xFE);
...
/*
* I2C read/write fuctions according to Wire library of arduino
*/
...
//--> Read all black magic calibration data from sensor
